...
# if defined _WIN32 || defined __HP_aCC // vc or b11_23u compile_type
plugins cl_dl; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# endif
#endif
 ...
 try {
	...
	dirpath += "/plugin";
	dirpath += "/image";
	std::string path_open(dirpath); path_open += "/open";
	std::string path_save(dirpath); path_save += "/save");
	path_open += "/tga.";
	path_save += "/tga.";
	const char *ext =
...
# endif
	...
	igs::image::reader *p_rdr = &tga_re;
	igs::tga::writer tga_wr;
	igs::image::writer *p_wtr = &tga_wr;
#endif
...
//------------------------------------------------------------
/******	std::ofstream ofs("tmp.raw",std::ios::binary);
	if (!ofs) {
		std::string str;
		str = "Can not open ";
		str += "tmp.raw";
	...
	for (;ext_number<p_wtr->ext_count();++ext_number) {
		if (p_wtr->ext_lists()[ext_number] == exten) {
			p_wtr->error_from_properties(
				prop,ext_number,error_or_warning_msg );
			break;
		}
	}
